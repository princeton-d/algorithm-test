...
// 입력값을 스페이스바( ) 기준으로 잘라주고 map 함수를 이용해 숫자열로 바꿔줌
const splitInput = input.split(' ', -1).map(Number);
// 체스 기물 기준값
const base: number[] = [1, 1, 2, 2, 2, 8];
// 체스 기물 기준값을 기준으로 map 함수를 사용해 입력값을 대입하여 빼준다.
const result = base.map((el, idx) => el - splitInput[idx]);
// 배열의 형태를 join 함수로 합쳐주고 스페이스바로 띄어준다.
